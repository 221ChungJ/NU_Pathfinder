...
//
// gets the location of the building by averaging the longitudes and latitudes of the nodes that make up the building's perimeter 
//
void Building::getLocation(Nodes nodes, double& avgLat, double& avgLon)
{
  ...
  for (long long nodeid : this->NodeIDs)
  {
    ...
    bool found = nodes.find(nodeid, lat, lon, dummy);
  ...
  avgLat = totalLat/numNodes; 
  avgLon = totalLon/numNodes; 
}
